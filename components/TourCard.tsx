      ...
      <div className="relative h-56 overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 flex gap-2">
            {tour.tags.slice(0, 2).map(tag => (
                <span key={tag} className="bg-black/60 backdrop-blur-md text-white px-2 py-1 rounded text-[10px] uppercase tracking-wider">
                    {tag}
                </span>
            ))}
        </div>
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex space-x-2 text-gray-400">
                    <span title="Documentary Evidence">
                        <Video className={`w-4 h-4 ${tour.documentaryUrl ? 'text-red-500' : ''}`} />
                    </span>
                    <span title="Academic Publication">
                        <BookOpen className={`w-4 h-4 ${tour.paperTitle ? 'text-blue-500' : ''}`} />
                    </span>
                </div>
            </div>
        </div>
